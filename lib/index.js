...
function registerSuggestedRepliesRpc(ctx, store, getConfig, setConfig, generateFn, dismissFn, forkFn, setCollapsedFn, getDraftFn) {
	ctx.connection.rpc.handle(CHANNEL, async (endpoint, payload, signal) => {
		switch (endpoint) {
			case "dock.setCollapsed":
				if (!isDockSetCollapsedPayload(payload)) return fail("payload must be { sessionId: string, collapsed: boolean }");
				try {
			...
			case "suggestions.fork":
				if (!isForkPayload(payload)) return fail("payload must be { sessionId: string, messageId: string, draft: string }");
				try {
					return ok({
						ok: true,
						sessionId: await forkFn(payload.sessionId, payload.messageId, payload.draft)
					});
				} catch (error) {
					return fail(error instanceof Error ? error.message : String(error));
				}
			case "draft.get":
				if (!isStateGetPayload(payload)) return fail("payload must be { sessionId: string }");
				return ok({ draft: getDraftFn(payload.sessionId) });
...
function isDockSetCollapsedPayload(value) {
	return isRecord(value) && typeof value.sessionId === "string" && value.sessionId.length > 0 && typeof value.collapsed === "boolean";
}
...
function isForkPayload(value) {
	return isRecord(value) && typeof value.sessionId === "string" && value.sessionId.length > 0 && typeof value.messageId === "string" && value.messageId.length > 0 && typeof value.draft === "string";
}
//#endregion
//#region src/state.ts
...
const suggestedRepliesRowSchema = z$1.object({
	...
	messageId: z$1.string().nullable().default(null),
	phase: z$1.union([
...
var SuggestedRepliesStateStore = class SuggestedRepliesStateStore {
	...
	*/
	...
	async setGenerating(session, turn, messageId, generationSessionId, isCurrent) {
	...
	async setReady(session, turn, messageId, generationSessionId, suggestions, isCurrent) {
		return await this.enqueue(session.id, async () => {
			if (!isCurrent()) return false;
			const current = this.currentRow(session.header);
...
function lastAssistantMessageIdForTurn(agent, turn) {
	const events = getSessionEvents(agent.session);
	let messageId = null;
	for (const event of events) if (event.type === "assistant/message" && event.data.turn === turn) messageId = event.data.message.id;
	return messageId;
}
/** Find the event seq of the assistant message with the given messageId. */
function seqForMessageId(agent, messageId) {
	const events = getSessionEvents(agent.session);
	for (const event of events) if (event.type === "assistant/message" && event.data.message.id === messageId) return event.seq;
	return null;
}
...
async function apply(ctx, config) {
	...
	const collapsedSessions = /* @__PURE__ */ new Set();
	/** Pending drafts for forked sessions: child sessionId → draft text. */
	const pendingDrafts = /* @__PURE__ */ new Map();
	let source = () => ({
	...
	if (settingsService !== void 0) settingsService.installSection(ctx, SETTINGS_NAMESPACE, SettingsSchema, source(), {
		...
		onChange: () => {
			if (source().suggestionCount === 0) clearAll().catch((error) => {
	...
	const startGenerationForSession = (agent, turn, _origin) => {
		...
		const task = runGeneration(ctx, store, internalSessions, gate, lease, agent, turn, messageId, request, getGenerationConfig()).catch((error) => {
	...
	registerSuggestedRepliesRpc(ctx, store, () => {
		const s = source();
		return {
			suggestionCount: s.suggestionCount,
			reasoningEffort: s.reasoningEffort,
		...
		return source();
	}, generateFn, dismissFn, async (sessionId, messageId, draft) => {
		const agent = ctx.agents.get(SessionId(sessionId));
		if (agent === void 0) throw new Error(`dsh-suggested-replies: session ${sessionId} not found for fork`);
		...
		if (collapsed) collapsedSessions.add(sessionId);
		else collapsedSessions.delete(sessionId);
	}, (sessionId) => {
		const draft = pendingDrafts.get(sessionId);
		if (draft !== void 0) pendingDrafts.delete(sessionId);
		return draft ?? null;
	});
	return async () => {
		disposing = true;
...
async function runGeneration(ctx, store, internalSessions, gate, lease, parent, turn, messageId, request, config) {
	...
	try {
		...
		await store.setReady(parent.session, turn, messageId, internalSessionId, suggestions, () => gate.isCurrent(lease));
	} catch (error) {
