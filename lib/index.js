...
function registerSuggestedRepliesRpc(ctx, store, getEnabled, setEnabled, generateFn, dismissFn) {
...
async function apply(ctx, config) {
	...
	if (settingsService !== void 0) settingsService.installSection(ctx, SETTINGS_NAMESPACE, SettingsSchema, { enabled: config.enabled }, {
		setSource: (next) => {
			source = next;
		},
		onChange: () => {
	...
	const startGenerationForSession = (agent, turn, _origin) => {
		...
		const task = runGeneration(ctx, store, internalSessions, gate, lease, agent, turn, request, generationConfig).catch((error) => {
